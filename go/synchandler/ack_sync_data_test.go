...
func readyAckSyncDataOK() *httptest.Server {
...
func TestHandlers_AcknowledgeSyncDataOK(t *testing.T) {
	testName := syncutil.GetCallingName()
	testhelper.StartTest(testName)
	// setupHTTPEnv()
	// defer teardownHTTPEnv()
	// syncutil.Debug("\nReady\n")
	server = readyAckSyncDataOK()
	//var reader io.Reader
	//Prepare a data for URL.
	requestData, err := testhelper.CreateSyncDataAck()
	if err != nil {
	...
	sessionURL := fmt.Sprintf(server.URL+
		"/ackData/sessionId/%s/nodeId/%s/transactionBindId/%s", sessionID, nodeID, transactionBindID)
